...
class MemoryContextBuilder:
    ...
    @staticmethod
    def build_context(
        memories: List[MemoryEntry], max_chars: int = 8000, limit: int = 50
    ) -> str:
        """
        ...
        for m in memories:
            if (
                limit < 10
                and m.category == "Identity"
                and m.predicate in ("COUNTRY", "STATE")
            ):
                continue
            ...
            if ver_list:
                ...
                for m in ver_list:
                    ...
                    if m.subject.lower() == "user":
                        if pred_clean == "name":
                            lines.append(
                                f"- The user's name: {m.object} (Confidence: {conf:.2f})"
                            )
                        elif pred_clean == "preferred name":
                            lines.append(
                                f"- The user prefers to be called: {m.object} (Confidence: {conf:.2f})"
                            )
                        elif pred_clean == "city":
                            lines.append(
                                f"- The user's city: {m.object} (Confidence: {conf:.2f})"
                            )
                        elif pred_clean == "state":
                            lines.append(
                                f"- The user's state: {m.object} (Confidence: {conf:.2f})"
                            )
                        elif pred_clean == "country":
                            lines.append(
                                f"- The user's country: {m.object} (Confidence: {conf:.2f})"
                            )
                        elif pred_clean == "lives with":
                            lines.append(
                                f"- The user lives with: {m.object} (Confidence: {conf:.2f})"
                            )
                        elif pred_clean == "lives independently":
                            lines.append(
                                f"- The user lives independently: {m.object} (Confidence: {conf:.2f})"
                            )
                        elif pred_clean == "works at":
                            lines.append(
                                f"- The user works at: {m.object} (Confidence: {conf:.2f})"
                            )
                        elif pred_clean == "founded":
                            lines.append(
                                f"- The user founded: {m.object} (Confidence: {conf:.2f})"
                            )
                        elif pred_clean == "works on":
                            lines.append(
                                f"- The user works on: {m.object} (Confidence: {conf:.2f})"
                            )
                        elif pred_clean == "has experience":
                            lines.append(
                                f"- The user has experience in: {m.object} (Confidence: {conf:.2f})"
                            )
                        elif pred_clean == "dislikes":
                            lines.append(
                                f"- The user dislikes: {m.object} (Confidence: {conf:.2f})"
                            )
                        elif pred_clean == "prefers":
                            lines.append(
                                f"- The user prefers: {m.object} (Confidence: {conf:.2f})"
                            )
                        elif pred_clean == "believes":
                            lines.append(
                                f"- The user believes: {m.object} (Confidence: {conf:.2f})"
                            )
                        elif pred_clean == "values":
                            lines.append(
                                f"- The user values: {m.object} (Confidence: {conf:.2f})"
                            )
                        elif pred_clean == "interested in":
                            lines.append(
                                f"- The user is interested in: {m.object} (Confidence: {conf:.2f})"
                            )
                        else:
                            lines.append(
                                f"- The user {pred_clean}: {m.object} (Confidence: {conf:.2f})"
                            )
                    else:
                        lines.append(
                            f"- {m.subject} {pred_clean}: {m.object} (Confidence: {conf:.2f})"
                        )
                lines.append("")
        ...
        while len(output_str) > max_chars and len(ranked_memories) > 1:
            ranked_memories.pop()
            verified = [
                m for m in ranked_memories if m.verification_status == "VERIFIED"
            ]
            pending = [
                m for m in ranked_memories if m.verification_status != "VERIFIED"
            ]
