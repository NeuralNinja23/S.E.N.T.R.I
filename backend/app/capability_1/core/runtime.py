...
from app.capability_1.core.contracts import MemoryEntry, EvidenceEntry, MemoryQuery, MemoryResult
from app.capability_1.core.registry import provider_registry
...
class MemoryRuntime:
    ...
    def remember(self, entry: MemoryEntry, turn_id: str) -> str:
        ...
        existing_id = self.exists(
            entry.category, entry.subject, entry.predicate, entry.object
        )

        if existing_id:
            logger.info(
                f"Fact '{entry.subject} {entry.predicate} {entry.object}' already exists (ID: {existing_id}). Recording evidence."
            )
            ...
            if existing_entry:
                ...
                if existing_entry.verification_status == "INFERRED":
                    ...
                    if evidence_count >= 3:
                        logger.info(
                            f"Promoting memory {existing_id} to VERIFIED due to reinforcement ({evidence_count} observations)."
                        )
                        ...
                        other_verified = self.provider.query(
                            MemoryQuery(
                        ...
                        for ov in other_verified:
                            if ov.id != existing_id:
                                logger.info(
                                    f"Deleting older verified fact (ID: {ov.id}, Object: {ov.object}) due to promotion of new fact."
                                )
        ...
        candidates = self.provider.query(
            MemoryQuery(
        ...
        should_upsert_overwrite = (
            entry.category in ("Identity", "Career") and len(candidates) > 0
        )
        ...
        if should_upsert_overwrite:
            ...
            logger.info(
                f"New candidate value for {entry.category}/{entry.subject}/{entry.predicate}: {entry.object}. Storing as PENDING (INFERRED)."
            )
    ...
    def exists(
        self, category: str, subject: str, predicate: str, object_val: str
    ) -> Optional[str]:
