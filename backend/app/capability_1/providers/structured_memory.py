...
from app.capability_1.core.contracts import MemoryEntry, EvidenceEntry, MemoryQuery
...
class StructuredMemoryProvider:
    ...
    def save(self, entry: MemoryEntry) -> str:
        ...
        if importance == 0.5:
            if entry.category == "Identity" and entry.predicate in (
                "NAME",
                "PREFERRED_NAME",
            ):
                importance = 1.0
            elif entry.category == "Identity" and entry.predicate in (
                "CITY",
                "STATE",
                "COUNTRY",
            ):
