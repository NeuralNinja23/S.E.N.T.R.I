...
class SQLiteStore:
    ...
    def initialize_db(self):
        ...
        if "semantic_importance" not in columns:
            cursor.execute(
                "ALTER TABLE memory_entries ADD COLUMN semantic_importance REAL NOT NULL DEFAULT 0.5"
            )

        # Update existing records with default importance weights
        cursor.execute(
            "UPDATE memory_entries SET semantic_importance = 1.0 WHERE category = 'Identity' AND predicate IN ('NAME', 'PREFERRED_NAME')"
        )
        cursor.execute(
            "UPDATE memory_entries SET semantic_importance = 0.85 WHERE category = 'Identity' AND predicate IN ('CITY', 'STATE', 'COUNTRY')"
        )
        cursor.execute(
            "UPDATE memory_entries SET semantic_importance = 0.9 WHERE category = 'Career'"
        )
        cursor.execute(
            "UPDATE memory_entries SET semantic_importance = 0.9 WHERE category = 'Project'"
        )
        cursor.execute(
            "UPDATE memory_entries SET semantic_importance = 0.9 WHERE category = 'Goal'"
        )
        cursor.execute(
            "UPDATE memory_entries SET semantic_importance = 0.8 WHERE category = 'Preference'"
        )
        cursor.execute(
            "UPDATE memory_entries SET semantic_importance = 0.7 WHERE category = 'Lifestyle'"
        )
    ...
    def exists_entry(
        self, category: str, subject: str, predicate: str, val_object: str
    ) -> Optional[str]:
