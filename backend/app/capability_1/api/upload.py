...
async def upload_document(file: UploadFile = File(...)):
    ...
    try:
        ...
        uploaded_documents[:] = [
            doc for doc in uploaded_documents if doc["filename"] != filename
        ]

        # Append parsed details
        uploaded_documents.append(
            {"filename": filename, "content": content, "size": file_size}
        )

        logger.info(
            f"Successfully uploaded and parsed document: {filename} ({file_size} bytes)"
        )
