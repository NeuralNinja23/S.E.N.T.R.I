...
class QuickResponseEngine:
    ...
    def _is_identity_question(self, query: str) -> bool:
        ...
        identity_keywords = ("who are you", "what are you", "your name", "who is sentri", "what is sentri", "tell me your name", "whats your name")
        return any(k in query for k in identity_keywords) or (("who" in query or "what" in query) and "name" in query and "your" in query)

    def _is_name_question(self, query: str) -> bool:
        # Match anything asking for user's own name
        return ("my name" in query or "who am i" in query or "do you know me" in query) and any(w in query for w in ("what", "whats", "who", "do", "tell"))

    def _is_time_question(self, query: str) -> bool:
        # Match any variation of asking for the current time
        return "time" in query and any(w in query for w in ("what", "whats", "tell", "current", "now", "is it"))

    def _is_date_question(self, query: str) -> bool:
        # Match any variation of asking for the current date/day
        return any(k in query for k in ("what day", "what date", "today's date", "whats today", "what is today")) or (("date" in query or "day" in query) and any(w in query for w in ("what", "whats", "tell", "today")))

    def _is_thanks(self, query: str) -> bool:
        return query in (
            "thank you", "thanks", "thanks a lot", "thank you very much",
            "thank you sentri", "thanks sentri", "much appreciated",
        )

    def _is_goodbye(self, query: str) -> bool:
        return query in (
            "goodbye", "bye", "bye bye", "see you", "see you later",
            "good night", "goodnight", "see ya", "later",
        )
