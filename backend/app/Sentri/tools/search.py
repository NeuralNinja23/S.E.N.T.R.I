...
def search_code(query: str, search_type: str = "text", path: str = ".") -> str:
    ...
    if search_type == "class":
        ...
        regex = re.compile(
            r"(?:def|function)\s+([A-Za-z0-9_]+)\b|const\s+([A-Za-z0-9_]+)\s*=\s*(?:async\s*)?\("
        )
    else:
        # text, symbol, comment
                        ...
                        if search_type in ("class", "function"):
                            ...
                            for match in matches:
                                ...
                                m_str = (
                                    ...
                                    else match[1] if isinstance(match, tuple) else match
                                )
                                if (
                                    isinstance(m_str, str)
                                    and query.lower() in m_str.lower()
                                ):
                                    file_matches.append(
