...
def list_directory(path: str = ".") -> str:
    ...
    if not target_path.exists():
        return json.dumps(
            {
                "error": f"Directory does not exist: {path}. Try using get_file_tree() or list_directory('.') to find the correct path."
            }
        )
    ...
    try:
        for item in target_path.iterdir():
            ...
            if len(items) >= 200:
                items.append(
                    {
                        "name": "... [TRUNCATED AT 200 ITEMS] Use search_code instead ...",
                        "type": "warning",
                        "size": None,
                    }
                )
                break
        return json.dumps({"path": str(target_path.as_posix()), "items": items})
    except Exception as e:
...
def read_file(path: str) -> str:
    ...
    if not target_path.exists():
        return json.dumps(
            {
                "error": f"File does not exist at exactly '{path}'. You must provide the full relative path from the workspace root. Run get_file_tree() or search_code() to find the correct path."
            }
        )
    if not target_path.is_file():
        return json.dumps(
            {"error": f"Path is a directory, use list_directory instead: {path}"}
        )
        ...
        try:
            ...
            for page_num, page in enumerate(reader.pages):
                ...
                if total_chars > 50000:
                    ...
                    break
            return json.dumps(
                {"path": str(target_path.as_posix()), "content": "\n".join(text_parts)}
            )
        except ImportError:
            return json.dumps(
                {
                    "error": "This is a PDF file. To enable PDF text extraction, please run 'pip install pypdf' in the backend environment."
                }
            )
    ...
    if target_path.suffix.lower() == ".docx":
        try:
            ...
            return json.dumps(
                {
                    "error": "This is a DOCX file. To enable DOCX text extraction, please run 'pip install python-docx' in the backend environment."
                }
            )
    ...
    if is_binary_file(target_path):
        return json.dumps(
            {
                "error": f"File is binary and cannot be read as text: {path}. Use open_externally to open it."
            }
        )
    ...
    try:
        ...
        return json.dumps({"path": str(target_path.as_posix()), "content": content})
    except Exception as e:
        return json.dumps({"error": str(e)})
        ...
        for root, dirs, files in os.walk(search_root):
            ...
            for f in files:
                if not f.startswith(".") or f in (".env", ".gitignore"):
                    ...
                    if len(tree) >= 400:
                        tree.append(
                            "... [TRUNCATED AT 400 FILES] Use search_code to find specific files ..."
                        )
                        return json.dumps(
                            {
                                "tree": tree,
                                "warning": "Directory too large to fully map.",
                            }
                        )
...
def open_externally(path_or_url: str = None, path: str = None) -> str:
    ...
    try:
        ...
        if target.startswith(
            ("http://", "https://", "mailto:", "ms-settings:", "ms-calculator:")
        ):
            os.startfile(target)
            return json.dumps(
                {
                    "status": "success",
                    "message": f"Successfully launched URL/URI: {target}",
                }
            )
        ...
        if not target_path.exists():
            ...
            try:
                ...
                return json.dumps(
                    {
                        "status": "success",
                        "message": f"Successfully launched system command: {target}",
                    }
                )
            except Exception as cmd_err:
                return json.dumps(
                    {
                        "error": f"Resource not found: {target}. (Launch error: {cmd_err})"
                    }
                )
        ...
        return json.dumps(
            {"status": "success", "message": f"Successfully opened: {target_path}"}
        )
    except Exception as e:
