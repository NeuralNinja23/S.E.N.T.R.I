...
async def handle_tool_call(function_call, websocket: WebSocket) -> str:
    ...
    logger.info(
        f"Intercepted tool call: {tool_name} | Mode: {TOOL_EXECUTION_MODE.get(tool_name, 'SPECIAL')}"
    )
    ...
    try:
        await websocket.send_json(
            {"type": "system", "message": f"TOOL_CALL: {tool_name} ({args_str})"}
        )
    ...
    if tool_name == "check_status":
        ...
        events_list = [
            {"task_id": e.task_id, "type": e.type, "result": e.result} for e in unread
        ]
        ...
        return json.dumps({"active_tasks": active, "new_notifications": events_list})
    ...
    if not mode:
        ...
        try:
            await websocket.send_json(
                {"type": "system", "message": f"TOOL_ERROR: {tool_name} -> {err_msg}"}
            )
        except Exception:
            pass
    ...
    if mode == "background":
        ...
        try:
            ...
            try:
                await websocket.send_json(
                    {"type": "system", "message": f"TOOL_ERROR: {tool_name} -> {e}"}
                )
            except Exception:
                pass
            raise e
