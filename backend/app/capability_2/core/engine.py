...
class ConversationEngine:
    ...
    async def run_voice_turn(
        ...
        if hasattr(self.model, "process_audio_stream_with_text"):
            ...
            response_gen = self.model.process_audio_stream_with_text(
                input_audio_stream, **params
            )
            ...
            async for output_chunk in self.model.process_audio_stream(
                input_audio_stream
            ):
                yield {"type": "audio", "data": output_chunk}

    @staticmethod
    async def run_text_turn(system_prompt: str, text_query: str, websocket=None) -> str:
        """
        Executes a text reasoning query by routing to the adapter.
        """
        return await ConversationAdapter.generate_async(
            system_prompt=system_prompt, user_content=text_query, websocket=websocket
        )
