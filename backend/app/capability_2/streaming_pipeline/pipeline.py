...
from app.capability_2.core.interfaces import ISpeechToSpeechModel
from app.capability_2.core.contracts import (
    TurnContext,
    TurnChannels,
)
from app.capability_2.events.event_bus import (
    EventBus,
    SpeechStarted,
    SpeechFinished,
    TranscriptReady,
...
from app.capability_2.prompts.prompt_builder import PromptBuilder
from app.capability_2.prompts.system_prompt import SystemPromptProvider
from app.capability_2.prompts.memory_provider import MemoryProvider
from app.capability_2.streaming_pipeline.providers.registry import ProviderRegistry
from app.capability_2.streaming_pipeline.chunker import SpeechPlanner
...
class RuntimeSupervisor:
                ...
                for group, group_tasks in self.workers.items():
                    for name, task in group_tasks.items():
                        if task.done() and not task.cancelled():
                            ...
                            if exc:
                                logger.error(
                                    f"[SUPERVISOR] Worker {group}/{name} failed with exception: {exc}"
                                )
                                self.cancel_all()
                                return
    ...
    def cancel(self, target: str = "all"):
        ...
        """
        logger.info(
            f"[SUPERVISOR] Hierarchical cancellation requested for target: {target}"
        )
...
class ConversationRuntime(ISpeechToSpeechModel):
    ...
    def __init__(self):
        ...
        self.reasoning = ProviderRegistry.get_reasoning(
            REASONING_PROVIDER, model_name=REASONING_MODEL
        )
        ...
        from app.capability_2.routing.quick_responses import QuickResponseEngine
    ...
    def _register_event_loggers(self):
        self.event_bus.subscribe(
            SpeechStarted, lambda e: logger.info(f"[{e.turn_id}] SpeechStarted")
        )
        self.event_bus.subscribe(
            SpeechFinished, lambda e: logger.info(f"[{e.turn_id}] SpeechFinished")
        )
        self.event_bus.subscribe(
            TranscriptReady,
            lambda e: logger.info(f"[{e.turn_id}] TranscriptReady: '{e.text}'"),
        )
        self.event_bus.subscribe(
            ReasoningStarted, lambda e: logger.info(f"[{e.turn_id}] ReasoningStarted")
        )
        self.event_bus.subscribe(
            TokenGenerated,
            lambda e: logger.debug(f"[{e.turn_id}] TokenGenerated: '{e.token}'"),
        )
        self.event_bus.subscribe(
            ChunkReady, lambda e: logger.info(f"[{e.turn_id}] ChunkReady: '{e.text}'")
        )
        self.event_bus.subscribe(
            AudioStarted, lambda e: logger.info(f"[{e.turn_id}] AudioStarted")
        )
        self.event_bus.subscribe(
            AudioFinished, lambda e: logger.info(f"[{e.turn_id}] AudioFinished")
        )
        self.event_bus.subscribe(
            Interrupted, lambda e: logger.info(f"[{e.turn_id}] Interrupted")
        )
        self.event_bus.subscribe(
            Cancelled, lambda e: logger.info(f"[{e.turn_id}] Cancelled")
        )
        self.event_bus.subscribe(
            Error, lambda e: logger.error(f"[{e.turn_id}] Error: {e.message}")
        )
    ...
    async def process_audio_stream_with_text(
        ...
        output_queue: asyncio.Queue[Tuple[str, Union[bytes, str, None]]] = (
            asyncio.Queue()
        )
        ...
        if quick_response:
            logger.info(
                f"[{turn_id}] Quick response: '{quick_response}' (intent={intent})"
            )
        ...
        async def llm_worker():
            ...
            context.clock.llm_start_time = time.time()
            try:
                async for token in self.reasoning.stream(
                    reasoning_request, websocket=websocket
                ):
            ...
            try:
                while not context.cancel_token.is_set():
                        ...
                        if remaining:
                            ...
                            if cleaned_remaining:
                                self.event_bus.publish(
                                    ChunkReady(turn_id=turn_id, text=cleaned_remaining)
                                )
                                await channels.phrase_queue.put(
                                    (time.time(), cleaned_remaining)
                                )
                        # Signal TTS complete
                    ...
                    async for phrase in self.speech_planner.feed(token):
                        ...
                        if cleaned_phrase:
                            self.event_bus.publish(
                                ChunkReady(turn_id=turn_id, text=cleaned_phrase)
                            )
                            await channels.phrase_queue.put(
                                (time.time(), cleaned_phrase)
                            )
            except asyncio.CancelledError:
                pass
            except Exception as e:
                ...
                if token_delays:
                    ...
                    logger.debug(
                        f"[{turn_id}] SpeechPlanner Avg Token Queue Delay: {avg_delay_ms:.2f}ms"
                    )
            ...
            try:
                while not context.cancel_token.is_set():
                    ...
                    async for audio_chunk in self.tts.synthesize(phrase):
                        ...
                        self.event_bus.publish(
                            AudioChunkEvent(turn_id=turn_id, pcm_bytes=audio_chunk)
                        )
                ...
                if phrase_delays:
                    ...
                    logger.debug(
                        f"[{turn_id}] TTS Avg Phrase Queue Delay: {avg_delay_ms:.2f}ms"
                    )
        ...
        async def playback_worker():
            ...
            try:
                ...
                if audio_delays:
                    ...
                    logger.debug(
                        f"[{turn_id}] Playback Avg Audio Queue Delay: {avg_delay_ms:.2f}ms"
                    )
        ...
        try:
            ...
            t_prompt = (
                context.clock.prompt_built_time - context.clock.asr_end_time
            ) * 1000
            t_ttft = (
                context.clock.first_token_time - context.clock.llm_start_time
            ) * 1000
            t_ttfa = (
                context.clock.first_audio_frame_time - context.clock.speech_end_time
            ) * 1000
    ...
    async def process_audio_stream(
        ...
        """
        async for event_type, content in self.process_audio_stream_with_text(
            audio_generator
        ):
            if event_type == "audio":
