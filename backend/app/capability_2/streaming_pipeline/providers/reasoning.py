...
class ReasoningProvider(ABC):
    ...
    @abstractmethod
    async def stream(
        self, request: ReasoningRequest, websocket=None
    ...
    async def stream(
        ...
        for msg in request.history:
            ...
            mapped_history.append(
                {"role": role, "content": msg.get("text", "") or msg.get("content", "")}
            )
            ...
            try:
                async with self._client.stream("POST", url, json=payload) as response:
                    if response.status_code != 200:
                        logger.error(
                            f"Ollama endpoint returned HTTP error {response.status_code}"
                        )
                        return
                    ...
                    class ThinkTagStripper:
                        ...
                        def feed(self, token: str) -> str:
                            ...
                            if self.in_think:
                                ...
                                if any(
                                    self.buffer.endswith(self.buffer[:i])
                        ...
                        for tc in chunk_tool_calls:
                            ...
                            if tc_args:
                                if isinstance(tc_args, dict):
                                    if not isinstance(
                                        target["function"]["arguments"], dict
                                    ):
                    ...
                    for tc in tool_calls:
                        ...
                        if isinstance(args, str) and args.strip():
                            try:
                                ...
                                logger.error(
                                    f"Failed to parse streamed tool call arguments JSON: {parse_err}"
                                )
                                func_info["arguments"] = {}
                ...
                if tool_calls:
                    ...
                    for tool_call in tool_calls:
                        ...
                        logger.info(
                            f"[VOICE TURN] Ollama requested tool: {name}({args_str})"
                        )
                        ...
                        logger.info(
                            f"[VOICE TURN] Tool {name} result: {result[:200]}"
                        )
