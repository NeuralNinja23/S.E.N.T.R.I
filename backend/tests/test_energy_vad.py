        ...
        if rms > threshold:
            ...
            if is_user_speaking:
                ...
                if silence_chunks >= 38:  # 1.2s of silence
                    ...
                    print(f"[{i * 0.032:.2f}s] Silence threshold met. Speech COMPLETED.")
                    
    print("\n--- Energy VAD Summary ---")
    print(f"Speech segments detected: {len(speech_starts)}")
    for start, end in zip(speech_starts, speech_ends + ([total_chunks * 0.032] if len(speech_starts) > len(speech_ends) else [])):
