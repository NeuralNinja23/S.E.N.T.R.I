...
async def scenario_1_rapid_bargeins():
    ...
    async with websockets.connect(WS_URL) as ws:
        await send_audio(ws, main_pcm, label, turn_complete=False)  # no turn_complete — barge-in mid-stream
...
async def main():
    ...
    required = ["greeting_hello", "query_name", "query_projects",
                "query_time", "query_vram", "barge_in_stop", "barge_in_wait",
                "compound_morning", "cancel_restart_1", "cancel_restart_2", "cancel_restart_3"]
    ...
    with open(out, "w", encoding="utf-8") as f:
        ...
        f.write(f"**Audio:** Kokoro TTS (af_sarah, en-gb, 16 kHz PCM16)\n\n")
        f.write("## Summary\n\n| Scenario | Result |\n|---|---|\n")
        for name, status in summary:
