...
class TestBug12_SQLiteFKCascade(unittest.TestCase):
    ...
    def test_cascade_delete_removes_evidence(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                len(store.get_evidence_by_memory_id("mem-001")),
                0,
...
class TestBug17_NPlus1RecallUpdates(unittest.TestCase):
    ...
    def test_batch_update_called_once(self):
        from app.capability_1.providers.structured_memory import StructuredMemoryProvider
        ...
        with patch.object(
            ...
            StructuredMemoryProvider, "batch_update_recall_time"
        ) as mock_batch:
            r = MemoryRuntime()
...
class TestBug23_ASRDiagnosticsRemoved(unittest.TestCase):
    ...
    def test_asr_debug_logging_code_removed(self):
        ...
        import inspect
        from app.capability_2.streaming_pipeline.providers import asr
...
class TestBug30_ToolSchemaCategory(unittest.TestCase):
    ...
    def test_category_in_schema(self):
        ...
        schema = next(
            s for s in TOOL_SCHEMAS if s["function"]["name"] == "remember_fact"
        )
        props = schema["function"]["parameters"]["properties"]
        required = schema["function"]["parameters"]["required"]
        self.assertIn(
            "category", props, "'category' missing from remember_fact properties"
        )
        self.assertIn(
            "category", required, "'category' missing from remember_fact required list"
        )
...
class TestBug3_ChatHistory(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_history_in_payload(self):
        ...
        with patch("httpx.AsyncClient.post", side_effect=fake_post):
            await ConversationAdapter.generate_async(
                "System", "Who am I?", history=history
            )
    ...
    async def test_think_tags_are_stripped(self):
        ...
        with patch.object(provider._client, "stream", return_value=ctx_manager):
            ...
            async for tok in provider.stream(
                ReasoningRequest(system_prompt="", user_input="Hi")
            ):
                tokens.append(tok)
...
class TestBug6_TokenCap(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_token_cap_in_payload(self):
        ...
        self.assertEqual(
            ...
            1024,
            "num_predict must be 1024 in voice options",
        )
...
class TestBug24_TTSLanguage(unittest.TestCase):
    ...
    def test_tts_language_in_source_not_hardcoded(self):
        import inspect
        from app.capability_2.streaming_pipeline.providers import tts as tts_mod
...
class TestBug1_VoiceTranscriptRewrite(unittest.TestCase):
    ...
    def test_rewrite_code_absent(self):
        import inspect
        from app.capability_2.streaming_pipeline import pipeline
...
class TestBug4_TrueStreaming(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_stream_true_in_payload(self):
        ...
        self.assertTrue(
            captured["json"].get("stream"),
            "Payload must have stream=True for real streaming",
        )
...
class TestBug10_ASRLanguageConfigured(unittest.TestCase):
    ...
    def test_asr_calls_config_language(self):
        import inspect
        from app.capability_2.streaming_pipeline.providers import asr as asr_mod
...
class TestBug13_SubstringFalsePositives(unittest.TestCase):
    ...
    def test_bare_ram_word_triggers_ram_usage(self):
        ...
        self.assertTrue(
            bool(re.search(r"\bram\b", "what is my ram usage")),
            "Word-boundary regex must match standalone 'ram'",
        )
        self.assertFalse(
            bool(re.search(r"\bram\b", "i am programming")),
...
class TestBug14_VRAMNotShadowed(unittest.TestCase):
    ...
    def test_vram_is_gpu_question_not_ram(self):
        ...
        self.assertTrue(
            engine._is_gpu_question("what is my vram usage"),
            "VRAM query must be classified as a GPU question",
        )
        self.assertFalse(
            engine._is_ram_question("what is my vram usage"),
    ...
    def test_vram_respond_calls_gpu_handler(self):
        ...
        engine = QuickResponseEngine()
        with patch.object(
            engine, "_gpu_usage", return_value="GPU stats"
        ) as mock_gpu, patch.object(
            engine, "_ram_usage", return_value="RAM stats"
        ) as mock_ram:
...
class TestBug16And26_CasingParity(unittest.TestCase):
    ...
    def test_uppercase_predicate_matches(self):
        ...
        import inspect
        from app.capability_2.routing import quick_responses

        source = inspect.getsource(
            quick_responses._user_name
            if hasattr(quick_responses, "_user_name")
    ...
    def test_title_case_category_used_in_query(self):
        ...
        with patch("app.capability_1.core.runtime.MemoryRuntime") as MockRuntime:
...
class TestBug19_IntentGreetingDetection(unittest.TestCase):
    ...
    def test_regex_pattern_at_module_level(self):
        ...
        import app.capability_2.routing.intent_analysis as ia_mod

        self.assertTrue(
            hasattr(ia_mod, "_GREETING_ONLY"),
...
class TestBug29_RedactorMultiWord(unittest.TestCase):
    ...
    def test_non_credential_key_not_redacted(self):
        ...
        self.assertIn(
            "en-gb", result, "Non-credential keys like 'language' must not be redacted"
        )
...
class TestBug11_ConsolidatedCleaner(unittest.TestCase):
    ...
    def test_utils_file_exists(self):
        import app.capability_2.infra.utils as utils_mod
