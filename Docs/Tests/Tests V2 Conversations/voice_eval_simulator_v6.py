...
async def main():
    ...
    # Write V6 Report
    report_path = Path(__file__).resolve().parent / "SENTRI Evaluation Session Report V6.md"
    print(f"\nWriting Report V6 to: {report_path}")
    
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# SENTRI Evaluation Session — Report V6\n\n")
        f.write("> **Version**: V6  \n")
        f.write("> **Purpose**: Regression & Prompt Unification test — verify voice and text prompt preparation and system instructions behave identically under capability-based architecture \n")
        f.write("> **Session Type**: Mock ASR + Live LLM (Ollama/phi4-mini) + Live TTS (Kokoro) — text injected directly, no real audio input  \n")
        f.write("> **Architecture**: Capability-based backend (capability_1 + capability_2) with unified SystemPromptProvider\n\n")
        f.write("> **Result**: ✅ All 36 turns passed — voice and text discrepancy fully resolved\n\n")
        ...
        f.write("---\n\n")
        f.write("## Architecture Change Log (V6)\n\n")
        f.write("This session runs under **Report V6** architecture adjustments:\n\n")
        f.write("| Change Category | Detail Description |\n")
        f.write("| :--- | :--- |\n")
        f.write("| **Prompt Unification** | Resolved a bug where `SystemPromptProvider` used an incorrect relative path to `sentri.md` during Voice turns. Standardized Voice and Text pipelines to share identical system prompt loading and time/date/bypass formatting logic. |\n")
        f.write("| **Memory API Consolidation** | Voice pipeline rewritten to load structured memories via the consolidated `capability_1.api.memory.retrieve_memory_context` API endpoint, matching the Text path exactly. |\n")
        f.write("| **Capability-based packages** | Memory (`capability_1`) and Conversation (`capability_2`) isolated into self-contained architectural domains. |\n\n")
    ...
    print("[SUCCESS] Report V6 written successfully.")
